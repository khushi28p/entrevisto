...
export async function POST(req: NextRequest) {
  ...
  try {
      ...
      return NextResponse.json(
        {
          success: false,
          message:
            "Failed to extract text from PDF. Ensure your resume is text-selectable (not a scanned image).",
        },
        { status: 400 }
      );
    }
      ...
      return NextResponse.json(
        {
          success: false,
          message:
            "Parsed resume text is too short. Your PDF may be a scanned image or improperly formatted. Please upload a text-based PDF.",
        },
        { status: 400 }
      );
    }
    ...
    const filename = `${clerkId}/${Date.now()}-${file.name.replace(
      /[^a-zA-Z0-9.]/g,
      "_"
    )}`;
