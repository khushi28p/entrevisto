...
import { Briefcase, BrainCircuit, Users, ChevronRight, TrendingUp, FileText, AlertCircle, Clock, CheckCircle } from 'lucide-react';
...
const StatusBadge = ({ status }: { status: string }) => {
  const statusConfig = {
    ...
    AI_SCREENING_COMPLETE: { color: "bg-green-500/10 text-green-500", label: "Screening Done" },
  ...
  >
    <div className="flex justify-between items-start mb-2">
      <div className="flex-1">
        ...
        <p className="text-sm text-muted-foreground">{application.jobPosting.company.name}</p>
      </div>
    ...
    {application.interviewResult && (
      <div className="mt-3 pt-3 border-t border-border/50">
        <div className="flex items-center justify-between">
          ...
          <span className="text-sm font-bold text-chart-2">{application.interviewResult.score}/100</span>
        </div>
      </div>
    )}
  </Link>
...
export default async function CandidateDashboardPage() {
  ...
  const avgScore = practiceInterviews.length > 0
    ? Math.round(practiceInterviews.reduce((sum, i) => sum + i.score, 0) / practiceInterviews.length)
    : 0;
          ...
          <div className="bg-orange-500/10 border border-orange-500/20 rounded-xl p-4 flex items-start space-x-3 mb-8">
            ...
            <div className="flex-1">
              ...
              <p className="text-sm text-muted-foreground mt-1">
                Upload your resume to start applying for jobs and unlock personalized recommendations.
              </p>
            </div>
            <Link href="/candidate/practice">
              ...
              <div className="bg-card border border-border rounded-xl p-6 shadow-sm">
                <div className="flex items-center justify-between mb-4">
                  ...
                  {applications.length > 3 && (
                    <Link href="/candidate/applications" className="text-xs text-chart-2 hover:underline">
                      View All
                    </Link>
                  )}
                </div>
          ...
          <div className="lg:col-span-2 space-y-6">
            ...
            {activeJobs.length > 0 ? (
              ...
              <div className="text-center p-12 text-muted-foreground border-2 border-dashed border-border/50 rounded-xl bg-card/50">
                ...
                <p className="text-sm mt-1">Check back later or start a practice session!</p>
              </div>
            )}
          </div>
        </div>
