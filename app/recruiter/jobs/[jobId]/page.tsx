  ...
  return (
    ...
    >
      <div className="flex items-start justify-between mb-3">
        <div>
          <p className="font-semibold text-foreground">{application.candidate.user.email}</p>
          <p className="text-xs text-muted-foreground mt-1">
            Applied: {new Date(application.appliedAt).toLocaleDateString()}
          </p>
        </div>
        <span className={`px-3 py-1 text-xs font-semibold rounded-full ${statusColors[application.status as keyof typeof statusColors]}`}>
      ...
      {application.interviewResult && (
        <div className="flex items-center justify-between pt-3 border-t border-border/50">
          ...
          <span className={`text-sm font-bold ${getScoreColor(application.interviewResult.score)}`}>
...
export default async function JobDetailsPage({ params }: PageProps) {
  ...
  const completedScreenings = job.applications.filter(
    app => app.status === "AI_SCREENING_COMPLETE" || 
           app.status === "REVIEWED_BY_RECRUITER" ||
           app.status === "OFFERED" ||
           app.status === "REJECTED"
  ).length;
  const averageScore = job.applications
      ...
      <div className="max-w-6xl mx-auto space-y-8 pt-8">
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
          ...
          <div className="p-6 bg-card border border-border rounded-xl shadow-sm">
            ...
            <p className="text-3xl font-bold text-foreground">{completedScreenings}</p>
          </div>
